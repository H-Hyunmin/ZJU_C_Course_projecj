...
void draw_bmp(const char* bmp_path, double x, double y, double width, double height) {
    ...
    // 加载位图文件
    //这个函数是用来加载位图文件的，它的参数包括文件路径、位图类型、宽度和高度等信息。
    //具体来说，它会返回一个 HBITMAP 句柄，该句柄可以用于创建一个位图对象，然后可以将该位图对象绘制到设备上下文中。
    ...
    //内存设备上下文是一个虚拟的设备上下文，而是在内存中维护一个设备上下文，可以用于绘制图形、文本等。
    ...
    //SelectObject函数将位图对象(hBitmap)选入到内存设备上下文(memDC)中，以便后续的绘制操作可以在该设备上下文中进行。
    SelectObject(memDC, hBitmap);

    // 获取位图信息
    //BITMAP是一个结构体，用于存储位图的信息，包括位图的宽度、高度、颜色位数等。
    //这个变量在后续的代码中被用来存储从位图文件中加载的位图信息。
    BITMAP bmp;
    //这个函数是用来获取位图对象的信息的，它的参数包括位图对象的句柄、获取信息的大小和存储信息的缓冲区等。
    GetObject(hBitmap, sizeof(BITMAP), &bmp);

    // 设置透明色为白色
    COLORREF crTransparent = RGB(255, 255, 255);  // 白色

    // 使用 TransparentBlt 绘制位图，透明色为白色
    //这个函数是 TransparentBlt，它是一个 Windows API 函数，用于在设备上下文中绘制一个位图，并且可以指定透明色。
    //它的参数包括目标设备上下文、目标矩形、源设备上下文、源矩形、透明色等。在这个代码中，它的作用是将加载的位图绘制到屏幕上，并且指定白色为透明色。具体来说，它的参数如下：
    //osdc：目标设备上下文，即屏幕设备上下文。
    //px、py：目标矩形的左上角坐标，即位图在屏幕上的位置。
    //pwidth、pheight：目标矩形的宽度和高度，即位图在屏幕上的大小。
    //memDC：源设备上下文，即内存设备上下文。
    //0、0：源矩形的左上角坐标，即从位图的左上角开始绘制。
    //bmp.bmWidth、bmp.bmHeight：源矩形的宽度和高度，即绘制整个位图。
    //crTransparent：透明色，即白色
